...
def editorial_card(a, base=""):
    """Large editorial card for deep-dive articles on featured page."""
    ...
    type_lbl = e(a.get("type_label","ANALYSIS"))
    summary  = e((a.get("dek") or a.get("card_summary") or a.get("meta_description",""))[:220])
    date_str = d(a.get("published",""))
    rt       = read_time(a.get("word_count",1200))

    return f"""    <article class="editorial-card">
      <div class="editorial-card-img">
        <a href="{href}" style="display:block;height:100%;">
          <img src="{e(img_url)}" alt="{title}" loading="lazy"
               onerror="this.src=\'{fallback}\'">
        </a>
      </div>
      <div class="editorial-card-body">
        <span class="editorial-card-type">{type_lbl}</span>
        <h3 class="editorial-card-title"><a href="{href}">{title}</a></h3>
        <p class="editorial-card-summary">{summary}</p>
        <div class="editorial-card-footer">
          <span>&#128337; {rt}</span>
          <span>&middot;</span>
          <span>{date_str}</span>
          <a href="{href}" class="editorial-card-read">Read full analysis &rarr;</a>
        </div>
      </div>
    </article>"""
...
{nav(a['cat_page'], base="../")}
  <main>
    <div style="max-width:760px;margin:0 auto;padding:40px 24px 80px;">
      ...
      {f'''<div style="margin-top:24px;padding:20px 22px;background:#fff;border:2px solid #e8e8ed;border-radius:12px;">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:10px;">
          <div style="width:44px;height:44px;border-radius:50%;background:linear-gradient(135deg,#8b95a5,#5856d6);display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;">📡</div>
          <div><strong style="font-size:14px;color:#1d1d1f;">The Streamic Editorial Team</strong>
          <div style="font-size:12px;color:#86868b;">Broadcast &amp; Streaming Technology</div></div>
        </div>
        <p style="font-size:13px;color:#555;line-height:1.65;margin:0;">
          The Streamic editorial team covers broadcast IP, streaming infrastructure, post-production technology, and newsroom systems. Our analysis is written by engineers and producers with direct experience of the technology we cover, without vendor sponsorship or affiliate relationships.
          <a href="../about.html" style="color:{color};font-weight:600;margin-left:4px;">Learn more &#8594;</a>
        </p>
      </div>''' if a.get("editorial") else ''}
...
def _bento_item(a, base=""):
    """SSR bento-grid-item for SEO — matches the JS createCardHtml() structure exactly."""
    href    = f"{base}articles/{a['slug']}.html"
    img_url = e(a.get("image_url",""))
    title   = e(a.get("title",""))
    summary = e((a.get("card_summary") or a.get("dek") or a.get("meta_description",""))[:280])
...
def home_with_hero(feat_arts: list, all_arts: list) -> str:
    ...
    # Split: editorial deep-dives first, then RSS grid
    editorial_arts = [a for a in all_arts if a.get("editorial")][:5]
    # Exclude both editorial and the hero from the grid
    hero_slug = hero.get("slug","") if hero else ""
    rss_arts = [a for a in grid_arts if not a.get("editorial") and a.get("slug") != hero_slug][:20]
    ssr_cards = "\n".join(_bento_item(a) for a in rss_arts[:12])

    editorial_html = ""
    if editorial_arts:
        cards_edit = "\n".join(editorial_card(a) for a in editorial_arts)
        editorial_html = f'''  <div class="page-wrap">
    <div class="page-section">
      <div class="section-hdr">
        <h2>&#9997;&#65039; Original Analysis &amp; Deep Dives</h2>
        <span class="ts-updated">Expert editorial from The Streamic</span>
      </div>
...
{cards_edit}
    ...
    cards_html = "\n".join(card(a, num=i+1) for i, a in enumerate(rss_arts))
...
{editorial_html}
    <div class="page-wrap">
      <div class="page-section">
        ...
        <!-- SSR bento grid for SEO; JS populates #bentoGridLarge from data/news.json -->
...
def main():
    ...
    # ── Prepare docs/data/news.json for client-side JS ──────────────────────
    # Transform data/news.json (flat list OR dict-of-categories) into
    # { featured_priority: [], items: [] } format that main.js expects.
    # This file MUST live inside docs/ so GitHub Pages can serve it.
    import json as _json
    _news_src  = os.path.join(ROOT, "data", "news.json")
    _news_dst  = os.path.join(DOCS, "data", "news.json")
    os.makedirs(os.path.dirname(_news_dst), exist_ok=True)
    if os.path.exists(_news_src):
        with open(_news_src, encoding="utf-8") as _f:
            _raw = _json.load(_f)
        # Normalise to flat list regardless of source format
        if isinstance(_raw, list):
            _flat = _raw          # fetch.py direct output
        elif isinstance(_raw, dict) and ("items" in _raw or "featured_priority" in _raw):
            _flat = _raw.get("items", []) + _raw.get("featured_priority", [])
        else:
            # dict-of-categories format from old fetch_rss.py
            _flat = []
            for _cat, _lst in _raw.items():
                for _it in (_lst if isinstance(_lst, list) else []):
                    if isinstance(_it, dict):
                        _it.setdefault("category", _cat)
                        _flat.append(_it)
        # Sort by date desc
        _flat.sort(key=lambda x: x.get("pubDate") or x.get("published") or "", reverse=True)
        # Split into featured_priority (top 6 newest) and items (rest)
        _featured = _flat[:6]
        _rest     = _flat[6:]
        _out = {"featured_priority": _featured, "items": _rest}
        with open(_news_dst, "w", encoding="utf-8") as _f:
            _json.dump(_out, _f, ensure_ascii=False)
        # Also copy to root/data/ for Pages-from-root fallback
        _root_data = os.path.join(ROOT, "data")
        os.makedirs(_root_data, exist_ok=True)
        with open(os.path.join(_root_data, "news.json"), "w", encoding="utf-8") as _f:
            _json.dump(_out, _f, ensure_ascii=False)
        print(f"  ✓ docs/data/news.json ready ({len(_featured)} featured + {len(_rest)} items)")
    else:
        # Create empty placeholder so JS doesn't throw
        _placeholder = {"featured_priority": [], "items": []}
        with open(_news_dst, "w") as _f:
            _json.dump(_placeholder, _f)
        print("  ⚠ data/news.json not found — empty placeholder written")
